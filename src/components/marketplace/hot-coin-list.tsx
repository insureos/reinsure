  ...
  return (
    <div className={className}>
      <Button
        ...
        className="w-auto !p-0 hover:bg-transparent focus:bg-transparent [&>span.drip]:hidden"
      >
        {wishListed ? (
                ...
                {item.tags.length
                  ? item.tags.map((tag, index) => (
                      <Text
                        key={`tag-${index}`}
                        tag="span"
                        className="max-w-[170px] truncate rounded-full px-3 py-1 text-xs bg-gray-800"
                      >
                        {tag}
                      </Text>
                    ))
