          ...
          <div key={item.id} className="text-start">
            ...
            {isActive && (
              <AnimatePresence>
                ...
                >
                  <Text className="ps-8 text-sm leading-6 text-gray-400">
                    {item.content}
                  </Text>
                </motion.div>
              </AnimatePresence>
...
export default function DCAStepper() {
  ...
  return (
    <div className="w-full md:w-[680px]">
      <div className="relative flex flex-grow flex-col overflow-hidden rounded-lg p-4 shadow-card transition-all duration-200 hover:shadow-large bg-light-dark md:p-8">
        ...
        <div className="mt-13 flex items-end justify-between">
          <Link
            href="/"
            className="text-sm leading-6 hover:text-brand text-gray-300"
          >
            Help Center
          </Link>
