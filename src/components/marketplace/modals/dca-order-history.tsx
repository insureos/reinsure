...
import OpenOrders from '@/components/marketplace/open-orders';
...
const OrderHistory = dynamic(
  () => import('@/components/marketplace/order-history')
);
const Parameters = dynamic(() => import('@/components/marketplace/parameters'));
...
export default function DCAOrderHistory() {
  ...
  return (
    <div className="w-full xs:w-[440px] sm:w-[540px] md:w-[660px] ">
      <div className="relative flex flex-grow flex-col overflow-hidden rounded-lg bg-light-dark p-4 shadow-card transition-all duration-200 hover:shadow-large sm:min-h-[640px] md:min-h-[715px] md:p-8">
        <div className="mb-8 flex items-center justify-between border-b border-dashed border-gray-700 pb-6 text-left text-lg font-medium capitalize -tracking-wide text-white lg:text-xl">
          DCA BTC/USDT
          <Button
            title="Close"
            color="white"
        ...
        <div className="">
          ...
          >
            {isMounted && ['xs', 'sm'].indexOf(breakpoint) !== -1 ? (
              ...
              <Tab.List className="relative mb-9 flex h-13 justify-between rounded-full bg-[#0C0F19] px-1.5 text-sm shadow-2xl min-[375px]:justify-start min-[375px]:gap-6 2xl:gap-10">
                <TabItem
